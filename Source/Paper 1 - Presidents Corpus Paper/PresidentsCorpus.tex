...
\bigskip
\begin{abstract}
This paper looks at evidence of cognitive decline in three presidents speeches. Ronald Reagan, who would later go on to receive a diagnosis of Alzheimer's Disease and both George Herbert Walker Bush and Donald Trump who have no known diagnosis of dementia. This study used the methodology used by Berisha and Liss as a starting off point and offers some refinements in how the data was pre-processed, how the features were generated and how the analysis was conducted. We hypothesized that there would be features identified which would decline over time for RR, but not for our controls. We also hypothesised that a non-linear model would map decline more accurately than linear models. We collected data from the Question and Answer portions of the presidents press conferences and used Natural Language Processing to generate features for analysis. We found that there were a number of lexical features that significantly changed over time for RR but did not change for either GHWB or DJT. These features have a foundation in psychological literature and have been linked to cognitive decline in Alzheimer's Disease. Also, we found that non-linear models were able to more accurately map declines in RR's language over time. 
\end{abstract}
